import React from "react";
import AppointmentButton from "../AppointmentButton";
import { ImageExport } from "@/shared/images";

const Buttocks = () => {
  return (
    <div className="w-full 2xl:h-700 md:h-600 sm:h-500 h-full sm:bg-bgButtocks bg-none bg-no-repeat bg-cover 2xl:bg-bottom md:pt-24 sm:pt-14 pt-0 md:pl-16 sm:pl-5 pl-5 ">
      <div className="relative sm:hidden block">
        <img src={ImageExport.BUTTOCKSBG} alt="abs" className="" />
        <div className="absolute bottom-3 left-5">
          <h2 className="font-bold 2xl:text-3xl md:text-2xl text-3xl text-white font-DIN uppercase">
            Buttocks
          </h2>
          <p className="text-white font-Public_Sans font-normal md:text-base text-sm pt-3">
            HIFEM+ · Growth of Muscle
          </p>
        </div>
      </div>
      <div className="sm:w-400 w-full flex flex-col gap-3 px-5 sm:px-0">
        <h2 className="font-bold 2xl:text-4xl text-3xl text-white font-DIN uppercase hidden sm:block">
          Buttocks
        </h2>
        <p className="text-white font-Public_Sans font-normal md:text-base text-sm pb-3 hidden sm:block">
          HIFEM+ · Growth of Muscle
        </p>
        <p className="text-white font-Public_Sans font-normal md:text-base text-15p leading-7">
          A lot of fat is accumulated in the buttocks. The glutes can have
          layers of fat melted away by Emsculpt, giving the form a more
          attractive appearance. The patient's body seems leaner and stronger as
          a result of the glute muscles becoming more defined and apparent.
        </p>
      </div>
      <div className="pt-7 px-5 sm:px-0">
        <AppointmentButton href="/bookAppointment" />
      </div>
    </div>
  );
};

export default Buttocks;
